...
(function (factory) {
  ...
  //img class  add by zhengwei 2016-07-01
  var VIEWER_IMGS=".viewer-imgs";
  // Events
  ...
  Viewer.prototype = {
    ...
    init: function () {
      ...
      var $images = isImg ? $this : $this.find(VIEWER_IMGS);
     ...
     /* if (!length) {
      ...
      if (options.inline) {
        ...
        $this.on(EVENT_DBLCLICK, $.proxy(this.start, this));
      }
    },
    ...
    build: function () {
      ...
      //$this.after($viewer);
      //修改dom显示位置
      $("#"+options.appendBody).append($viewer);
    ...
    bind: function () {
      ...
      this.$viewer.on(EVENT_CLICK, $.proxy(this.click, this));
      this.$viewer.on(EVENT_WHEEL, $.proxy(this.wheel, this));
    ...
    initViewer: function () {
	  ...
	  //重新设置$images
	  this.$images=this.$element.find(VIEWER_IMGS);
    ...
    initList: function () {
      ...
      $list.html(list.join('')).find(VIEWER_IMGS).one(EVENT_LOAD, {
        filled: true
    ...
    start: function (e) {
      ...
      if ($(target).is('img')&&e.type=="dblclick"&&e.target.outerHTML.indexOf("viewer-imgs")>0) {
        this.target = target;
        this.show();
      }
    },
    ...
    update: function () {
      ...
      if (this.isImg) {
        ...
        this.$images = $images = $this.find(VIEWER_IMGS);
